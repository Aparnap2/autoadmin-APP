...
class SimpleAPIClient {
  ...
  /**
   * Initialize the client and connect to WebSocket if enabled
   */
  ...
  private async connectRealtime(): Promise<void> {
    try {
      ...
      const wsURL = this.config.baseURL.replace(/^https?/, wsProtocol) + '/api/v1/ws';

      // For React Native, we might need to use a different WebSocket implementation
      if (Platform.OS === 'web') {
        ...
        // For React Native, you would use a WebSocket library
        console.log('WebSocket connection for React Native would be implemented here');
      }
    } catch (error) {
  ...
  private handleRealtimeMessage(data: any): void {
    switch (data.type) {
      case 'message':
        this.notifyMessageListeners(data.message as ClientMessage);
        break;
      case 'task_update':
        this.notifyTaskListeners(data.task as ClientTask);
        break;
      case 'agent_status':
        this.notifyAgentListeners(data.agents as ClientAgent[]);
        break;
      default:
