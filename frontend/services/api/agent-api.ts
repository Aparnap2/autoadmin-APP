...
class AgentAPIService {
  ...
  constructor(config: Partial<TaskManagerConfig> = {}) {
    ...
    // Set up WebSocket listeners if real-time updates are enabled
  ...
  private setupWebSocketListeners(): void {
    this.fastapiClient.addWebSocketListener('task_update', (data) => {
      this.updateTaskCache(data);
    });

    this.fastapiClient.addWebSocketListener('agent_status', (data) => {
  ...
  async createTask(request: Omit<AgentTaskRequest, 'id' | 'created_at' | 'updated_at'>): Promise<AgentTaskResponse> {
    try {
      const response = await this.fastapiClient.createTask(request as AgentTaskRequest);
  ...
  async triggerAgentTask(request: Omit<AgentTaskRequest, 'id' | 'created_at' | 'updated_at'>): Promise<AgentTaskResponse> {
    try {
      const response = await this.fastapiClient.triggerAgentTask(request as AgentTaskRequest);
  ...
  async connectWebSocket(): Promise<void> {
    await this.fastapiClient.connectWebSocket();
  }

  // Disconnect WebSocket
  disconnectWebSocket(): void {
    this.fastapiClient.disconnectWebSocket();
  }
