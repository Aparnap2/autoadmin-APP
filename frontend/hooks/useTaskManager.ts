  ...
  useEffect(() => {
    const initService = async () => {
      try {
        const service = getAgentAPIService({
          syncWithBackend: options.syncWithBackend ?? true,
          enableRealTimeUpdates: options.enableRealtimeSync ?? true,
        });
        ...
        if (options.enableRealtimeSync) {
          ...
          service.addWebSocketListener('task_update', (data) => {
          ...
          service.addWebSocketListener('agent_status', (data) => {
    ...
    return () => {
      if (agentServiceRef.current) {
        agentServiceRef.current.disconnectWebSocket();
      }
