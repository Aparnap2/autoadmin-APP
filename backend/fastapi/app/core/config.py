...
class Settings(BaseSettings):
    ...
    FIREBASE_PROJECT_ID: str = Field(
        description="Firebase project ID", min_length=1
    )
    ...
    OPENAI_API_KEY: str = Field(
        description="OpenAI API key", min_length=1
    )
    ...
    OPENAI_MAX_TOKENS: int = Field(
        default=4096, description="Maximum OpenAI tokens"
    )
    ...
    UPLOAD_DIR: str = Field(
        default="./uploads", description="File upload directory"
    )
    ...
    ENABLE_METRICS: bool = Field(
        default=True, description="Enable application metrics"
    )
