...
class ChatMessage(BaseModel):
    ...
    tool_calls: Optional[List[Dict[str, Any]]] = Field(default=None, description="Tool calls data")
...
class ModelConfig(BaseModel):
    ...
    custom_parameters: Dict[str, Any] = Field(default_factory=dict, description="Custom provider parameters")


class ChatRequest(BaseModel):
    """Chat completion request model"""
    messages: List[ChatMessage] = Field(description="Chat messages")
    model_config: Optional[ModelConfig] = Field(default=None, description="Model configuration")
...
class EmbeddingRequest(BaseModel):
    ...
    model_config: Optional[ModelConfig] = Field(default=None, description="Model configuration")
    dimensions: Optional[int] = Field(default=None, ge=1, description="Embedding dimensions")
    user: Optional[str] = Field(default=None, description="User identifier")
    encoding_format: str = Field(default="float", description="Encoding format (float, base64)")
...
class VectorSearchRequest(BaseModel):
    ...
    model_config: Optional[ModelConfig] = Field(default=None, description="Model configuration for embedding generation")
...
class ModelStatus(BaseModel):
    ...
    last_checked: datetime = Field(default_factory=datetime.utcnow, description="Last status check")
...
class AISettings(BaseModel):
    ...
    max_tokens_per_request: int = Field(default=4096, ge=1, description="Max tokens per request")
    max_requests_per_minute: int = Field(default=60, ge=1, description="Max requests per minute")
    cost_limit_per_hour: float = Field(default=10.0, ge=0, description="Cost limit per hour")
    enable_caching: bool = Field(default=True, description="Enable response caching")
    cache_ttl_seconds: int = Field(default=3600, ge=0, description="Cache TTL in seconds")
    fallback_providers: List[LLMProvider] = Field(default_factory=list, description="Fallback providers")
