...
async def chat_completion(request: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to generate chat completion")
...
async def chat_completion_stream(request: ChatRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to start streaming chat completion")


@router.post("/completions", response_model=CompletionResponse, summary="Text Completion")
async def text_completion(request: CompletionRequest) -> CompletionResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to generate text completion")
...
async def generate_embeddings(request: EmbeddingRequest) -> EmbeddingResponse:
    ...
    try:
        ...
        cache_key = str(request.input) if isinstance(request.input, str) else str(hash(tuple(request.input)))
...
@router.post("/vector/search", response_model=VectorSearchResponse, summary="Vector Similarity Search")
    ...
    try:
        ...
        for doc_id, doc_data in collection.items():
            ...
            if similarity >= request.threshold:
                # Apply filters
                if request.filters:
                    if not all(doc_data.get(k) == v for k, v in request.filters.items()):
                        continue
...
async def list_models(
    provider: Optional[LLMProvider] = Query(default=None, description="Filter by provider"),
    model_type: Optional[ModelType] = Query(default=None, description="Filter by model type"),
    ...
    try:
        ...
        if model_type:
            model_statuses = [m for m in model_statuses if model_type.value in m.capabilities]
